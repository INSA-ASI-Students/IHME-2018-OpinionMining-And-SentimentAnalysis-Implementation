...
def fusion(train_filename, test_filename, predict_filename):
    ...
    opinion = np.hstack([np.array(dataset_train['Opinion Towards']), np.array(dataset_test['Opinion Towards'])])
    sentiment = np.hstack([np.array(dataset_train['Sentiment']), np.array(dataset_test['Sentiment'])])
    ...
    if predict_filename == '':
        data_test = data[round(data_length/2):round(data_length/2)+round(round(data_length/2)/2)]
